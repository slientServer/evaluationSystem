...
class RbacAccessController extends CommonController{
	public function index() {
		//列表过滤器，生成查询Map对象
		$map = $this->_search('Access');
	...
	public function edit() {
		...
		$node=M('node')->order('sort')->field($field)->select();//原有权限
		$access=M('access')->where(array('role_id'=>$rid))->getField('node_id',true);
		$node=node_merge($node,$access);//组装数组
	...
	public function update() {
		...
		$rid=I('role_id',0,'intval');
		$db=M('access');
		$db->where(array('role_id'=>$rid))->delete();//每次插入之前先清空
		//组合新权限数据
		$data=array();
