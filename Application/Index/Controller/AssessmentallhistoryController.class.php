...
class AssessmentallhistoryController extends CommonController{
	...
	public function index(){
		...
		foreach ($passedSixMonth as $key => $value) {
			# code...
			$queryRes[$key]= $assessment->join('es_user ON es_assessment.targetuser= es_user.id')->where($value)->field('es_user.showname, es_assessment.fromuser, es_assessment.score, es_assessment.remark, es_assessment.isanony, es_assessment.inserttime')->select();
			foreach ($queryRes[$key] as $key2 => $value) {
				# code...
				$fromuser_showname= $User->where(array('id' => $value['fromuser']))->field('showname')->find();
				$queryRes[$key][$key2]['fromuser_showname']= $fromuser_showname['showname'];
			}
		}
