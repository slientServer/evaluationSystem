...
class Rbac {
    ...
    static public function getAccessList($authId) {
        ...
        $sql    =   "select node.id,node.name,node.title,node.imgsrc,node.remark from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
        ...
        foreach($apps as $key=>$app) {
            ...
            //自己临时修改
            $access[strtoupper($appName)]['appInfo']   =  array('name' =>$app['name'] , 'title' =>$app['title'] , 'imgsrc'=> $app['imgsrc'], 'remark'=> $app['remark']);
            ...
            foreach($modules as $key=>$module) {
                ...
                //自己临时修改
                $access[strtoupper($appName)]['controller']= $moduleName;
                $access[strtoupper($appName)]['action']= $actionName? $actionName: 'index';
            }
        }
        return $access;
    }
